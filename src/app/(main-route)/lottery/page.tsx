  ...
  return (
    <div className="flex gap-2 text-center">
      {Object.entries(timeLeft).map(([unit, value]) => (
        <div key={unit} className="flex flex-col rounded-md bg-primary/10 p-2 min-w-[50px]">
          <span className="text-lg font-bold text-primary">{value}</span>
          <span className="text-[10px] uppercase text-muted-foreground">{unit.charAt(0)}</span>
        </div>
      ))}
    </div>
  );
...
export default function LotteryPage() {
  const [selectedLottery, setSelectedLottery] = useState<typeof lotteries[0] | null>(null);
    ...
    <div className="min-h-screen bg-muted/20 pb-20">
      ...
      <section className="relative overflow-hidden bg-primary py-16 text-primary-foreground lg:py-24">
        ...
        <div className="container relative mx-auto px-4 text-center">
          <Badge className="mb-4 bg-white/20 text-white hover:bg-white/30">Win Big Today</Badge>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            <Button size="lg" variant="outline" className="border-white hover:bg-white/10 rounded-full px-8">
              How it Works
            </Button>
          </div>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          <Card className="border-none shadow-lg">
            <CardContent className="flex flex-col items-center p-6 text-center">
              ...
              <p className="text-sm text-muted-foreground">Choose a lottery and purchase a ticket for as low as ৳10.</p>
            </CardContent>
          </Card>
          <Card className="border-none shadow-lg">
            <CardContent className="flex flex-col items-center p-6 text-center">
              ...
              <p className="text-sm text-muted-foreground">Watch the live draw countdown. Winners are picked randomly.</p>
            </CardContent>
          </Card>
          <Card className="border-none shadow-lg">
            <CardContent className="flex flex-col items-center p-6 text-center">
              ...
              <p className="text-sm text-muted-foreground">If your ticket number matches, you win the grand prize!</p>
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      <section className="container mx-auto mt-16 px-4">
        <Tabs defaultValue="active" className="w-full">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-3xl font-bold tracking-tight">Featured Lotteries</h2>
            <TabsList>
          ...
          <TabsContent value="active" className="space-y-6">
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {lotteries.filter(l => l.status === "active").map((lottery) => (
                <Card key={lottery.id} className="group overflow-hidden border-border/50 transition-all hover:shadow-xl hover:border-primary/50">
                  ...
                  <CardHeader>
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm font-medium text-primary mt-1">Win: {lottery.prize}</p>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="flex justify-between text-sm text-muted-foreground">
                      <span className="flex items-center gap-1"><Users className="h-4 w-4" /> {lottery.participants} Joined</span>
                      <span className="flex items-center gap-1"><Ticket className="h-4 w-4" /> {lottery.totalTickets - lottery.participants} Left</span>
                    </div>
                    <Progress value={(lottery.participants / lottery.totalTickets) * 100} className="h-2" />
                    
                    <div className="rounded-lg bg-muted/50 p-3">
                      <p className="mb-2 text-xs font-semibold uppercase text-muted-foreground text-center">Draw Starts In</p>
                  ...
                  <CardFooter>
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button className="w-full gap-2 text-lg font-semibold py-6 rounded-xl shadow-lg shadow-primary/20" onClick={() => setSelectedLottery(lottery)}>
                      ...
                      <DialogContent className="sm:max-w-[425px]">
                        <DialogHeader>
                          ...
                          <DialogDescription>
                            Enter your details to purchase a ticket for ৳{lottery.ticketPrice}.
                          </DialogDescription>
                        </DialogHeader>
                        <div className="grid gap-4 py-4">
                          ...
                          <div className="grid gap-2">
                            ...
                            <Input id="phone" type="tel" placeholder="017..." />
                          </div>
                          <div className="rounded-lg border p-3 bg-muted/30">
                            <div className="flex justify-between text-sm mb-1">
                              <span>Ticket Price</span>
                              <span className="font-semibold">৳{lottery.ticketPrice}</span>
                            </div>
                            <div className="flex justify-between text-sm mb-1">
                            ...
                            <div className="flex justify-between font-bold">
                              <span>Total</span>
                              <span className="text-primary">৳{lottery.ticketPrice}</span>
                            </div>
                          </div>
                        </div>
                        <DialogFooter>
                          <Button type="submit" className="w-full">Pay & Join</Button>
                        </DialogFooter>
                      </DialogContent>
                    </Dialog>
                  </CardFooter>
          ...
          <TabsContent value="completed">
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {lotteries.filter(l => l.status === "completed").map((lottery) => (
                <Card key={lottery.id} className="opacity-90">
                  ...
                  <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground">Prize: {lottery.prize}</p>
                  </CardHeader>
                  <CardContent>
                    <div className="rounded-xl bg-yellow-500/10 border border-yellow-500/20 p-4 flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-xs text-muted-foreground uppercase font-semibold">Winner</p>
                        <p className="font-bold text-foreground">{lottery.winner}</p>
                      </div>
                    </div>
                    <div className="mt-4 flex items-center gap-2 text-sm text-green-600">
                      <CheckCircle2 className="h-4 w-4" />
                      <span>Draw completed on {new Date(lottery.drawDate).toLocaleDateString()}</span>
                    </div>
                  </CardContent>
                </Card>
              ))}
        ...
        <div className="relative overflow-hidden rounded-3xl bg-black text-white">
          ...
          <div className="relative z-10 px-6 py-16 md:px-12 md:py-24">
            <div className="mx-auto max-w-3xl space-y-6">
              ...
              <h3 className="text-3xl font-bold sm:text-4xl">Watch Draws Live on Facebook & YouTube</h3>
              <p className="text-lg text-gray-300">
                         We ensure 100% transparency. Every draw is streamed live. You can verify the results instantly. 
